...
//void inputcostkey(unsigned char key,int x,int y);
...
int val;
bool paused=false;
bool enterpressed=false,resdisplay=false, gpressed=false;
char undostack[100];
char num[2];
...
//function useful for value and val key inputs 
...
void frontpage()
{
    ...
    bitmap_output(width/6+50,height-height/10, "JSS ACADEMY OF TECHNICAL EDUCATION",GLUT_BITMAP_TIMES_ROMAN_24);
    glEnd();
    glLineWidth(2);
    glColor3f(0.2,0.2,0.8);
    bitmap_output(width/5+50, height-height/6, "DEPARTMENT OF COMPUTER SCIENCE",GLUT_BITMAP_TIMES_ROMAN_24);
    glEnd();
    glColor3f(0.2,0.2,0.8);
    bitmap_output(width/5+50, height-height/4.3, "COMPUTER GRAPHICS MINI PROJECT",GLUT_BITMAP_TIMES_ROMAN_24);
    glEnd();
     glColor3f(0.0,0.0,0.0);
    bitmap_output(width/200+50,110,"Submitted By :",GLUT_BITMAP_TIMES_ROMAN_24);
    glEnd();
    glColor3f(0.6,0.2,0.8);
    bitmap_output(width/200+50,50,"Jyothsna P\t 1JS16CS045",GLUT_BITMAP_TIMES_ROMAN_24);
    glEnd();
    glColor3f(0.6,0.2,0.8);
    bitmap_output(width/250+50,80,"Greeshma Panchakshary\t 1JS16CS041",GLUT_BITMAP_TIMES_ROMAN_24);
    glEnd();
    glColor3f(0.9,0.4,0.6);
    bitmap_output(width/2.7+10, height-height/3.3, "WELCOME TO ",GLUT_BITMAP_TIMES_ROMAN_24);
    glColor3f(0.0,0.0,0.0);
    glBegin(GL_LINE_LOOP); 
        glVertex2f(width/3+30, height-height/3.2);
        glVertex2f(width/3+230, height-height/3.2);
    glEnd();
    glColor3f(0.9,0.4,0.6);
    bitmap_output(width/3+10, height-height/2.8, "Prims Agorithm Visualiser",GLUT_BITMAP_TIMES_ROMAN_24);
    glColor3f(0.0,0.0,0.0);
    glBegin(GL_LINE_LOOP); 
        glVertex2f(width/3-1, height-height/2.7);
        glVertex2f(width/3+290, height-height/2.7);
    glEnd();

    glColor3f(0.4,0.4,0.4);
    bitmap_output(width/3.5+10, height-height/2.0, "UNDER THE GUIDANCE OF :  ",GLUT_BITMAP_TIMES_ROMAN_24);
    glColor3f(0.0,0.0,0.0);
    glBegin(GL_LINE_LOOP); 
        glVertex2f(width/3-50, height-height/1.9);
        glVertex2f(width/3+320, height-height/1.9);
    glEnd();
    glColor3f(1.0,0.3,0.3);
    bitmap_output(width/3.7+10, height-height/1.7, "Mr.Mahesh Kumar M R.,B.E,MTech ",GLUT_BITMAP_TIMES_ROMAN_24);
    glEnd();

    glColor3f(1.0,0.3,0.3);
    bitmap_output(width/3.7+10, height-height/1.6, "Assistant Professor, Dept of CSE,",GLUT_BITMAP_TIMES_ROMAN_24);
    glEnd();
    
    glColor3f(1.0,0.3,0.3);
    bitmap_output(width/3.7+10, height-height/1.5, "JSSATE,Bengaluru ",GLUT_BITMAP_TIMES_ROMAN_24);
    glEnd();
    glutSwapBuffers();
    delay();
    glColor3f(0.0,1.0,0.0);
    bitmap_output(width/2+150, 30, "Press Enter to Continue.......",GLUT_BITMAP_HELVETICA_18);
    glutSwapBuffers();
    delay();
    glutPostRedisplay();
...
bitmap_output(10, height-height/3.7, "Note: Enter 2 digit cost before selecting nodes.",GLUT_BITMAP_HELVETICA_18);
...
int cost=0;
for(int i=0;i<pointer;i++){
if(oldx[i][0]>=(linepoints[0][0]-6)&&oldx[i][0]<=(linepoints[0][0]+6)){
if(oldx[i][1]>=(linepoints[0][1]-6)&&oldx[i][1]<=(linepoints[0][1]+6)){
      posi=i;
  break;}
   }
}
for(int i=0;i<pointer;i++){
if(oldx[i][0]>=(linepoints[1][0]-6)&&oldx[i][0]<=(linepoints[1][0]+6)){
if(oldx[i][1]>=(linepoints[1][1]-6)&&oldx[i][1]<=(linepoints[1][1]+6)){
      posj=i;
break;
}
   }
...
bitmap_output(20,50,"Enter cost, press G.",GLUT_BITMAP_HELVETICA_18);
glutSwapBuffers();
delay();
delay();delay();
delay();
...
costmatrix[posi][posj]=costmatrix[posj][posi]=val;
cost=val;
        }
}
...
sx=-1;sy=-1;
static int value[10],i=0;
switch(key){
    case 13 :found=0;
     enterpressed=true;
     val=array_to_num(value);
      i=0;
     break;
